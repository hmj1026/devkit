...
- [ ] 2.5 Port `Devkit\Core\Support\helpers.php`：純 PHP helpers (`isJson` 等)
...
- [ ] 5.1 Port `Devkit\Storage\Foundation\File` + `Image`
...
- [ ] 5.12 撰寫 Search 模組測試（mock Guzzle，斷言 ES request payload）

## 6. Wave 5 — Laravel 子命名空間（無 Bridge 前綴）

- [ ] 6.1 Port `Devkit\Laravel\Database\Entity\HasUuid`（trait） + 對應的 contract 實作
- [ ] 6.2 Port `Devkit\Laravel\Database\Entity\HasStatus`（trait） + 對應的 contract 實作
- [ ] 6.3 Port `Devkit\Laravel\Database\Entity\HasAuditLog`（trait） + 連動 `devkit-audit-logging`
- [ ] 6.4 撰寫 Entity traits 測試（Orchestra Testbench + SQLite in-memory）
- [ ] 6.5 撰寫 `Devkit\Laravel\Database\Criteria`（**選配查詢 helper**，非 Repository）
- [ ] 6.6 撰寫 `Devkit\Laravel\Database\Cast\EncryptedCast` + `HashedCast`（implements `CastsAttributes`）
- [ ] 6.7 撰寫 Casts 測試
- [ ] 6.8 Port `Devkit\Laravel\Storage\StorageAdapter`：給 disk name 回 `FilesystemOperator`
- [ ] 6.9 Port `Devkit\Laravel\Messaging\Sms\SmsChannel`（Notification channel） + `SendSmsJob`（queueable）
- [ ] 6.10 撰寫 SmsChannel + SendSmsJob 測試
- [ ] 6.11 Port `Devkit\Laravel\Http\Asset\HttpUriCacheAdapter`：wire `Illuminate\Cache\Repository`（PSR-16）到核心 HttpUri
- [ ] 6.12 Port `Devkit\Laravel\Http\Support\*`：`getClientTruthIp()`、`getUserClientIdCookie()`、`AbstractCookie`、`UserClientIdCookie`、`AccessLogMiddleware`、`AccessLogJob`、`ResponseLogJob`
- [ ] 6.13 Port `Devkit\Laravel\Logging\GoogleChat\GoogleChatLogServiceProvider`（`Log::extend('googlechat', ...)`）
- [ ] 6.14 Port `Devkit\Laravel\Ui\Trail\TrailServiceProvider` + Facade + `trail()` helper
- [ ] 6.15 Port `Devkit\Laravel\Ui\MetaTag\MetaTagServiceProvider` + Facade + `@meta_tags` Blade directive
- [ ] 6.16 新增 `Devkit\Laravel\Audit\AbstractEntityChangeLogger` trait + `EloquentLogTarget` + `ElasticsearchLogTarget`（含 jenssegers/agent 解析的 login event 支援）
- [ ] 6.17 撰寫 audit logging 測試（mock model event，斷言 log target payload）
- [ ] 6.18 Port `Devkit\Laravel\Queue\SqsFifo\SqsFifoQueue` + `SqsFifoConnector` + 4 個 Deduplicator + `SqsFifoQueueable` trait + ServiceProvider
- [ ] 6.19 連同來源 sqs-fifo-queue 套件的 tests/* 一起 port 並 namespace 替換
- [ ] 6.20 Port `Devkit\Laravel\Validation\Rules\TaiwanCellPhone` + `WithoutSpecialCharacters`，純 regex 邏輯抽到 `Devkit\Core\Validation\` 純 PHP 函式

## 7. Wave 6 — DX / ServiceProvider 索引 / Generator

- [ ] 7.1 撰寫 `Devkit\Laravel\DevkitServiceProvider`：條件式 register 各 module SP（透過 `config('devkit.modules.<name>.enabled')`）
- [ ] 7.2 撰寫 `config/devkit.php`：modules、disks、SMS drivers、HTTP gateway retry、ES connections、GoogleChat webhook、commands 開關
- [ ] 7.3 撰寫 **5 個** Artisan generator：
    - `devkit:make:service`
    - `devkit:make:action`
    - `devkit:make:enum`
    - `devkit:make:audit-log-target`
    - `devkit:make:http-client`
- [ ] 7.4 撰寫 `Devkit\Laravel\Command\InstallCommand` (`devkit:install`)：publish config + stub
- [ ] 7.5 撰寫每模組 docs/ 一份 md：使用情境、Laravel 設定範例、純 PHP 用法
- [ ] 7.6 撰寫頂層 docs/architecture.md：layer 圖、core 與 Laravel 子命名空間邊界、module 依賴圖
- [ ] 7.7 撰寫 docs/v2-roadmap.md：列出 v2 (PHP 8.1+) 預計引入的現代套件（Spatie 系列、Monolog 3、Flysystem 3 only 等）

## 8. Wave 7 — 收尾與驗收

- [ ] 8.1 跑 PHPUnit core + laravel testsuite，全綠
- [ ] 8.2 跑 GitHub Actions CI matrix，全綠
- [ ] 8.3 跑 Laravel Pint（或 PHP-CS-Fixer），全綠
- [ ] 8.4 `openspec validate bootstrap-devkit-toolkit` 通過
- [ ] 8.5 `openspec show bootstrap-devkit-toolkit` 顯示完整 artifact
- [ ] 8.6 從來源套件對照表逐項 checklist（11 套 → 14 capability → 已實作模組）
- [ ] 8.7 與用戶 review、依回饋微調
- [ ] 8.8 在既有專案做一次 dry-run import：composer require hmj1026/devkit + 改一個 class 引用為 Devkit\*，確認介面相容
- [ ] 8.9 標記 `bootstrap-devkit-toolkit` 為 ready-to-archive
- [ ] 8.10 後續開新 change `add-legacy-shim`：提供 `hmj1026/legacy-shim` 子套件 + 通用 `ClassAliasLoader`，讓既有專案漸進切換
